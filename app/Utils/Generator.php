...
class Generator
{
    ...
    public function procDistanceTime($from, $to, $ref, $mode, $hero = array(), $isreturn = FALSE)
    {
        ...
        if (!$mode) {
            ...
            if ($distance > TS_THRESHOLD) {
                if (!$isoasis && $this->getSortTypeLevel(14, $resarray) != 0) {
                    $speed = $speed * ((TS_THRESHOLD + ($distance - TS_THRESHOLD) * $bid14[$this->getSortTypeLevel(14, $resarray)]['attri'] / 100) / $distance);
                }
                if (!empty($hero)) {
    ...
    private function getSortTypeLevel($tid, $resarray)
    {
        $keyholder = [];
    ...
    public function getMilisecFormat($ms)
    {
        ...
        return sprintf("%02d:%02d:%02d (+ %d ms)", $hr, $min, $sec, $ms);
    }
